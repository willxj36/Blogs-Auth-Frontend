...
const all = async () => await Query('SELECT b.id, b.title, b.content, a.name as author, t.name as tag, b._created FROM blogs b JOIN authors a ON a.id = b.authorid JOIN blogtags bt ON bt.blogid = b.id JOIN tags t ON t.id = bt.tagid ORDER BY id DESC');

const one = async (id: number) => await Query('SELECT b.title, b.content, a.name as author, t.name as tag, b._created, b._updated FROM blogs b JOIN authors a ON a.id = b.authorid JOIN blogtags bt ON bt.blogid = b.id JOIN tags t ON t.id = bt.tagid WHERE b.id = ?', [id]);
